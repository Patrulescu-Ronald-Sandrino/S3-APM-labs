...
public class ProgramExecutionController implements Initializable {
    ...
    @FXML
    private TableColumn<Map.Entry<Integer, Integer>, String> latchTableTableViewLocationColumn;
    @FXML
    private TableColumn<Map.Entry<Integer, Integer>, String> latchTableTableViewValueColumn;
...
//          the object stored on that row of the TableView
    ...
    private void setLatchTableTableView() {
        latchTableTableView.refresh();

        latchTableTableViewLocationColumn.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getKey().toString()));
        latchTableTableViewValueColumn.setCellValueFactory(cellData -> new ReadOnlyStringWrapper(cellData.getValue().getValue().toString()));

        interpreter.getAnyProgramState().ifPresentOrElse(
                programState -> {
                    latchTableTableView.setItems(FXCollections.observableList(programState.getLatchTable().getContent().entrySet().stream().toList()));
                },
                () -> latchTableTableView.setItems(FXCollections.emptyObservableList())
        );
    }
